...
from plm_special.test_seq import Tester
...
#     Experience pool for collecting trajectories.
#     """
#     def __init__(self, exp_dataset_info=None):
#         self.states = []
#         self.actions = []
#         self.rewards = []
#         self.dones = []
#         self.exp_dataset_info = exp_dataset_info  # Store exp_dataset_info here if provided
...
def adapt(args, model, exp_dataset, exp_dataset_info, checkpoint_dir, best_model_dir, eval_process_reward_fn):
    ...
    # Tester(args, model, exp_pool , target_return, loss_fn, eval_process_reward_fn)


def test(args, model, exp_dataset, exp_dataset_info, checkpoint_dir, best_model_dir, eval_process_reward_fn):
    # exp_pool_path = "./data/exp_pools/exp_pool_with_cca_mapping.pkl"
    # exp_pool = pickle.load(open(exp_pool_path, 'rb'))
    # loss_fn = MSELoss()
    # model = load_model(args, model, model_dir)
    # target_return = exp_dataset_info.max_return * args.target_return_scale
    # print(f"Type of exp_dataset: {type(exp_dataset)}")
    # print(f"Value of exp_dataset: {exp_dataset}")

    # tester_obj = Tester(args, model=model, optimizer=optimizer, exp_dataset=exp_dataset, loss_fn=loss_fn, device=args.device, lr_scheduler=lr_scheduler, 
    #                 grad_accum_steps=args.grad_accum_steps)
    ...
    loss_fn = MSELoss()
    test_obj = Tester(args, model=model, optimizer=optimizer, exp_dataset=exp_dataset, loss_fn=loss_fn, device=args.device, lr_scheduler=lr_scheduler, 
                      grad_accum_steps=args.grad_accum_steps)
    ...
    for epoch in range(args.num_epochs):
        test_logs, test_losses = test_obj.test_epoch(epoch)
...
def run(args):
    ...
    if args.test:
        exp_pool_path = "./data/exp_pools/exp_pool_with_cca_mapping2.pkl"
    ...
    if args.test:
        ...
        test(args, model, exp_dataset, exp_dataset_info, checkpoint_dir, best_model_dir, process_reward)
