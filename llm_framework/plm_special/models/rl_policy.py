...
class OfflineRLPolicy(nn.Module):
    def __init__(
        ...
        self.action_head = nn.Sequential(nn.Linear(in_features=plm_embed_size, out_features=action_levels), nn.Tanh()).to(device)
        ...
        self.modules_except_plm = nn.ModuleList([  # used to save and load modules except plm
            ...
            self.embed_state1, self.embed_state2, self.embed_state3, self.embed_state4, self.embed_state5,self.action_head
        ])
    ...
    def forward(self, states, actions, returns, timesteps, attention_mask=None):
        ...
        for i in range(returns_embeddings.shape[1]):
            ...
            action_embed_positions[i] = (i + 1) * (2 + 5)
        ...
        print("rl_policy, action_pred",action_pred)
    ...
    def sample(self, state, target_return, timestep, **kwargs):
        ...
        print("rl_policy,,,action_levels",self.action_levels)
