...
def adapt(args, model, exp_dataset, exp_dataset_info, checkpoint_dir, best_model_dir, eval_process_reward_fn,max_action_value):
    optimizer = AdamW(
        model.parameters(),
        lr=args.lr,
    ...
    trainer = Trainer(args, model=model, optimizer=optimizer, exp_dataset=exp_dataset, loss_fn=loss_fn, device=args.device, lr_scheduler=lr_scheduler, 
                      grad_accum_steps=args.grad_accum_steps, max_action_value=max_action_value)
...
def run(args):
    ...
    exp_pool = pickle.load(open(exp_pool_path, 'rb'))
    max_action_value = np.max(exp_pool.actions)
    print("max action:",max_action_value)
    ...
    if args.adapt:
        ...
        adapt(args, rl_policy, exp_dataset, exp_dataset_info, checkpoint_dir, best_model_dir, process_reward,max_action_value)
    if args.test:
